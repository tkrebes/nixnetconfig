...
_sysapi_data = {
    'device1_port1_mock': {
        'ConnectsToLinkName': 'Device1 Link',
        'ExpertName': ['xnet'],
        'ExpertUserAlias': ['myPort1'],
        'IsDevice': False,
        'xnet': {
            'port_number': 1,
        },
    ...
    'device1_mock': {
        'ConnectsToLinkName': '',
        'ExpertName': ['xnet'],
        'ExpertUserAlias': ['Unknown'],
        'FirmwareRevision': '19072316',
        'IsDevice': True,
        'ProvidesLinkName': 'Device1 Link',
        'ProductName': 'NI PXI-8513',
        'SerialNumber': 'A2345678',
    },

    'chassis1_mock': {
        'ExpertUserAlias': ['myChassis'],
        'ExpertName': [''],
        'IsChassis': True,
        'ProvidesLinkName': 'chassis1',
        'ProductName': 'NI PXI chassis',
        'SerialNumber': 'A8765432',
    },
}
...
def get_nisyscfg_resource_filter_value(resource_cache, name):
    if name == 'ExpertName':
        return resource_cache['ExpertName'][0]
    if name == 'UserAlias':
        return resource_cache['ExpertUserAlias'][0]
    if name == 'SerialNumber':
        return resource_cache['SerialNumber']
...
class ResourceMock(object):
    def __init__(self, resource_cache):
        ...
        for expert in resource_cache['ExpertName']:
...
class SessionMock(object):
    ...
    def insert_device_to_chassis(self, chassis_name='chassis1_mock', device_name='device1_mock'):
        self._sysapi_cache[device_name]['ConnectsToLinkName'] = self._sysapi_cache[chassis_name]['ProvidesLinkName']

    def update_device_firmware_version(self, version, device_name='device1_mock'):
        self._sysapi_cache[device_name]['FirmwareRevision'] = version

    def find_hardware(self, **kwargs):
        filter = kwargs.copy()
        filter.pop('expert_names', None)
...
@mock.patch('nisyscfg2.Session', new_callable=SessionMock({}))
...
@mock.patch('sys.stdout', new_callable=io.StringIO)
@mock.patch('nisyscfg2.Session', new_callable=SessionMock(_sysapi_data))
def test_enumerate_xnet_devices_prints_device_info_when_xnet_devices_are_present(session_mock, stdout_mock):
    ...
    assert _sysapi_data['device1_mock']['ProductName'] in stdout_mock.getvalue()
    assert _sysapi_data['device1_port1_mock']['ExpertUserAlias'][0] in stdout_mock.getvalue()
...
def test_enumerate_xnet_devices_prints_device_info_when_xnet_devices_are_present_under_chassis(session_mock, stdout_mock):
    ...
    utilities.enumerate_xnet_devices()
    assert 'My System:\n' in stdout_mock.getvalue()
    assert _sysapi_data['chassis1_mock']['ExpertUserAlias'][0] in stdout_mock.getvalue()
    assert _sysapi_data['device1_mock']['ProductName'] in stdout_mock.getvalue()
    assert _sysapi_data['device1_port1_mock']['ExpertUserAlias'][0] in stdout_mock.getvalue()
...
def test_blink_xnet_port_sets_blink_property_and_invokes_save_changes(mode, expected_value):
    with mock.patch('nisyscfg2.Session', new_callable=SessionMock(_sysapi_data)) as session_mock:
